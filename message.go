	...
	if msg.Category != bot.MessageCategorySystemAccountSnapshot && msg.Category != bot.MessageCategorySystemConversation && msg.ConversationId == bot.UniqueConversationId(config.MixinClientId, msg.UserId) {
		if msg.Category == "PLAIN_TEXT" {
			...
			if "start" == string(data) {
				...
				filepath, _ := services.DownloadImage(ctx, url, "twitter")
				imageData, _ := services.CreateAttachment(ctx, filepath)
				if err := bot.PostMessage(ctx, msg.ConversationId, msg.UserId, bot.NewV4().String(), "PLAIN_IMAGE", imageData, config.MixinClientId, config.MixinSessionId, config.MixinPrivateKey); err != nil {
					log.Println("err", err)
				}
